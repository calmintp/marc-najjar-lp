...
<main x-data="{ activeVideo: null }">
            ...
            <div class="grid gap-10 md:grid-cols-2">
                @foreach($videos as $video)
                    <div class="group">
                        ...
                        <button 
                            @click="activeVideo = '{{ $video->youtube_id }}'"
                            class="relative block w-full overflow-hidden rounded-2xl border border-border/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                        ...
                        >
                            ...
                            <img 
                                src="{{ $video->thumbnail }}" 
                                alt="{{ $video->title }}" 
                                class="aspect-video w-full object-cover brightness-75 transition-all duration-500 group-hover:brightness-50 group-hover:scale-105"
                            >
                            ...
                            <div class="absolute inset-0 flex items-center justify-center">
                                <div class="flex size-16 items-center justify-center rounded-full border-2 border-primary bg-primary/20 text-primary backdrop-blur-sm transition-all duration-300 group-hover:scale-110 group-hover:bg-primary/40">
                                    <svg class="size-6 fill-primary" viewBox="0 0 24 24">
                        ...
                        <h3 class="mt-4 font-serif text-xl font-bold text-foreground">
                            {{ $video->title }}
                        </h3>

                        <p class="mt-2 leading-relaxed text-sm text-muted-foreground">
                            {{ $video->description }}
                        </p>
    ...
    <template x-teleport="body">
        ...
        >
            ...
            <button 
                @click="activeVideo = null"
                class="absolute right-4 top-4 z-50 flex size-10 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20"
